...
def config_setup():
    ...
    # if (model_config["cont_model_name"] == "bert-base-uncased") and (
    #     model_config["cand_model_name"] == "prajjwal1/bert-tiny"
    # ):
    #     cont_trf_config = AutoConfig.from_pretrained(
    #         model_config["cont_model_name"], hidden_size=128, num_attention_heads=8
    #     )
    #     cand_trf_config = AutoConfig.from_pretrained(model_config["cand_model_name"])

    # elif model_config["cont_model_name"] == "prajjwal1/bert-tiny" and (
    #     model_config["cand_model_name"] == "bert-base-uncased"
    # ):
    #     cand_trf_config = AutoConfig.from_pretrained(
    #         model_config["cand_model_name"], hidden_size=128, num_attention_heads=8
    #     )
    #     cont_trf_config = AutoConfig.from_pretrained(model_config["cont_model_name"])
    # else:
    cand_trf_config = AutoConfig.from_pretrained(
        model_config["cand_model_name"], hidden_size=128, num_attention_heads=8
    )
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_config["cont_model_name"], do_lower_case=True, clean_text=False
    )
    if model_config["shared"] is True:
        cont_bert = AutoModel.from_config(cont_trf_config)
        cand_bert = cont_bert
        cand_trf_config = None
        print("shared model created")
    else:
        cont_bert = AutoModel.from_config(cont_trf_config)
        cand_bert = AutoModel.from_config(cand_trf_config)
        print(
            f"""creating non shared model with:
            \tCandidate model : {model_config["cand_model_name"]}
            \tContext model : {model_config["cont_model_name"]}"""
        )
    model = BiEncoder(
        cand_config=cand_trf_config,
        cont_config=cont_trf_config,
        cont_bert=cont_bert,
        cand_bert=cand_bert,
            ...
            if candidate_type == "embedding":
                ...
                for lbl, cand in zip(cand_embed, candidates):
                    if model_config["loss_type"] == "cos":
                        ...
                        out_data["context"] = cont
                        out_data["candidate"].append(cand)
                        out_data["score"].append(
                            float(cosine_sim(vec_a=data, vec_b=lbl))
                        )
    ...
    try:
        ...
        for data in dataset.keys():
            for dat in dataset[data]:
                ...
                train_data["candidates"].append(data.lower())
                train_data["labels"].append(1)
                if train_config["train_with_neg"] is True:
                    while True:
                        choice = random.choice(list(dataset.keys()))
                        if choice != data:
                            train_data["contexts"].append(dat)
                            train_data["candidates"].append(choice.lower())

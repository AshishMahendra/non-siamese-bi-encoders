...
def train_model(model, tokenizer, contexts, candidates, labels,
                train_config, arch="bi"):
    ...
    concat_transform = token_util.SelectionConcatTransform(
        tokenizer=tokenizer,
        max_len=train_config['max_candidate_length']+train_config[
            'max_contexts_length'])
    train_dataset = token_util.SelectionDataset(
        ...
        concat_transform=concat_transform, arch=arch
    )
            ...
            for step, batch in enumerate(train_dataloader, start=1):
                ...
                if arch == 'cross':
                    context_token_ids_list_batch, \
                        ...
                        context_segment_ids_list_batch, labels_batch = batch
                    loss = model(context_token_ids_list_batch,
                                 context_input_masks_list_batch,
                                 context_segment_ids_list_batch, labels_batch)
                else:
                    context_token_ids_list_batch, \
                        context_input_masks_list_batch, \

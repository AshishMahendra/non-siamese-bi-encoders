...
def config_setup():
    ...
    model = PolyEncoder(config=trf_config,
                        cont_bert=cont_bert,
                        cand_bert=cand_bert,
...
def infer(contexts: Union[List[str], List[List]],
    ...
    try:
        if (context_type == "text") and (candidate_type == "text"):
            predicted_candidates.append(get_inference(
                model=model, contexts=contexts,
                candidates=candidates,
                tokenizer=tokenizer, train_config=train_config))
